...
{services.includes('translation') && <div className="option-group compact-options"><b>{t.targetLang}</b><div className="option-chips">{Object.keys(languageKeys).map(id => {
              ...
              return compatibleFormats.includes(id) ? <button type="button" key={id} className={outputFormats.includes(id) ? 'active' : ''} onClick={() => toggle(id, outputFormats, setOutputFormats)}>{outputFormats.includes(id) && <Check />}{label}</button> : null;
                ...
                ...outputOptions,
                [id]: e.target.checked
              })} /><span><b>{label}</b></span></label>)}</div>}<div className="bilingual-layout-control"><b>{isZh ? '双语输出布局' : 'Bilingual output layout'}</b><HoverSelect value={outputOptions.bilingual_layout || 'auto'} onChange={e => setOutputOptions({
              ...outputOptions,
              bilingual_layout: e.target.value
            })}><option value="auto">{isZh ? 'AI 自动推荐（推荐）' : 'AI recommended'}</option><option value="inline">{isZh ? '同行显示' : 'Inline'}</option><option value="vertical">{isZh ? '上下显示' : 'Vertical pairs'}</option><option value="columns">{isZh ? '左右分列' : 'Side-by-side columns'}</option><option value="target-only">{isZh ? '仅输出译文' : 'Translation only'}</option></HoverSelect>{(outputOptions.bilingual_layout === 'columns' || outputOptions.bilingual_layout === 'auto') && <div className="bilingual-layout-secondary"><label><span>{isZh ? '分列样式' : 'Column style'}</span><HoverSelect value={outputOptions.columns_style || 'address-with-text'} onChange={e => setOutputOptions({
                  ...outputOptions,
                  columns_style: e.target.value
                })}><option value="address-with-text">{isZh ? '标签+原文 ｜ 标签+译文（PLC/HMI 推荐）' : 'Address + source | Address + translation'}</option><option value="address-separate">{isZh ? '标签 ｜ 原文 ｜ 标签 ｜ 译文' : 'Address | Source | Address | Translation'}</option><option value="text-only">{isZh ? '原文 ｜ 译文' : 'Source | Translation'}</option></HoverSelect></label>{hasSpreadsheetFiles && <label><span>{isZh ? 'PLC/HMI 标签保留' : 'PLC/HMI label handling'}</span><HoverSelect value={outputOptions.address_mode || 'keep'} onChange={e => setOutputOptions({
                  ...outputOptions,
                  address_mode: e.target.value
                })}><option value="keep">{isZh ? '标签与文字连在一起（推荐）' : 'Keep address with text'}</option><option value="separate">{isZh ? '标签单独一列' : 'Put address in a separate column'}</option><option value="hide">{isZh ? '仅输出文字，不输出标签' : 'Text only, omit address'}</option></HoverSelect></label>}</div>}{outputOptions.bilingual_layout === 'inline' && <div className="bilingual-layout-secondary"><label><span>{isZh ? '同行样式' : 'Inline style'}</span><HoverSelect value={outputOptions.inline_style || 'dash'} onChange={e => setOutputOptions({
                  ...outputOptions,
                  inline_style: e.target.value
                })}><option value="dash">{isZh ? '原文 - 译文' : 'Source - translation'}</option><option value="slash">{isZh ? '原文 / 译文' : 'Source / translation'}</option><option value="parentheses">{isZh ? '原文（译文）' : 'Source (translation)'}</option><option value="pipe">{isZh ? '原文 | 译文' : 'Source | translation'}</option></HoverSelect></label></div>}{outputOptions.bilingual_layout === 'vertical' && <div className="bilingual-layout-secondary"><label><span>{isZh ? '上下顺序' : 'Vertical order'}</span><HoverSelect value={outputOptions.vertical_order || 'source-first'} onChange={e => setOutputOptions({
                  ...outputOptions,
                  vertical_order: e.target.value
                })}><option value="source-first">{isZh ? '原文在上（推荐）' : 'Source first'}</option><option value="target-first">{isZh ? '译文在上' : 'Translation first'}</option></HoverSelect></label></div>}<small>{isZh ? files.some(f => /\.(xlsx?|csv)$/i.test(f.name)) ? 'Excel/PLC/HMI 默认推荐“标签+原文｜标签+译文”，标签不会参与翻译。' : 'AI 会根据文件类型选择最佳排版。' : 'Excel/PLC/HMI defaults to address-preserving side-by-side columns.'}</small></div><button type="button" className="advanced-subtoggle" onClick={() => toggleAdvancedSection('document')}><span>{isZh ? '文档附加信息' : 'Document metadata'}</span><ChevronDown className={advancedSections.document ? 'open' : ''} /></button>{advancedSections.document && <div className="output-options-panel"><label className="output-option-row"><input type="checkbox" checked={!!outputOptions.preserve_comments} onChange={e => setOutputOptions({
                ...outputOptions,
